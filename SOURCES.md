...
- **Citation points at our archive, not at the report page (shipped 2026-08-25).** Every ABC citation is now `/source/<sha256>/#row-<n>` — the archived report bytes a post was read from, rendered for a person, with the live abc.ca.gov page linked from it as the authority. The tracker (Tier A) and the weekly narrative (Tier B, via the bundle's `allowedUrls`, so the citation gate can only clear a URL that resolves) both use it.
- **The archive page renders an EXTRACT, and says so.** Reports are statewide: the 2026-08-12 status-changes report held 410 rows across 183 cities, two of them in Chino. The page shows the rows whose premises city we cover, states how many of how many, and links the full report. Each row's **mailing address and escrow agent are omitted** — a mailing address is frequently a sole proprietor's home, and neither column backs a claim published here. Recorded in EDITORIAL.md under "Archive-backed citations".
- **The anchor is the row's position in the report, not the licence number.** 78 of those 410 rows repeat a licence number (a licence can change status more than once in one report), so a number-based anchor would send a citation about one status change to another. `meta.row_index` carries the position; items ingested before 2026-08-25 have none and their citations degrade to the whole-document page rather than to a fragment that matches nothing.
- **Backfilling already-published tracker posts is still open.** Changing a published post's `sources` is a content edit and needs the visible-correction treatment; posts generated from 2026-08-25 carry the new citation automatically.
...
- **Quirks & Policy (see reports/notes/dailybulletin-news.md):**
  - Fail-closed robots compliance (`failClosedRobots: true`) and redirect protection.
  - Terms of service tracked in `source_tos_status` with weekly drift checks (`scripts/check-tos-drift.ts`). Each check archives the page it hashes, so a drift can be read as a diff: `node scripts/check-tos-drift.ts --diff dailybulletin-news`.
