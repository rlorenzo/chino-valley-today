...
function readPostBody(post: PostRow): string {
	...
	if (!existsSync(abs))
		return "_(file missing at expected path — DB/filesystem are out of sync)_";
...
function renderPublished(db: Db): string {
	...
	const rows = posts
        ...
        <td>${
					flags.length
						? flags
								.map((f) => `<span class="badge badge-fail">${esc(f)}</span>`)
								.join(" ")
...
function renderAudit(db: Db): string {
	...
	const postTypes = Array.from(
		new Set(
			(
				db.raw
					.prepare(`SELECT DISTINCT post_type FROM posts WHERE tier = 'B'`)
					.all() as Array<{
					post_type: string;
				}>
			).map((r) => r.post_type),
	...
	const rows = sampled
		.map((p) => {
			const existing = db.raw
				.prepare(
					`SELECT verdict, notes, audited_at FROM audit_log WHERE post_id = ? AND iso_week = ?`,
				)
...
export function createApp(db: Db) {
	...
	app.post("/posts/:slug/approve", async (c) => {
		...
		if (post.status !== "held") {
			return c.text(
				`post ${slug} is not in the held queue (status=${post.status})`,
				400,
			);
		}
	...
	app.post("/audit/:slug", async (c) => {
		...
		if (
			...
			post.held_reason != null
		) {
			return c.text(
				`post ${slug} is not eligible for audit (tier=${post.tier}, status=${post.status})`,
				400,
			);
		}
		const week = isoWeekString();
		if (!isSampledForAudit(slug, week)) {
			return c.text(
				`post ${slug} is not in this week's (${week}) audit sample`,
				400,
			);
		}
		const existing = db.raw
			...
			.get(post.id, week);
		if (existing)
			return c.text(`post ${slug} was already audited for ${week}`, 400);

		const form = await c.req.parseBody();
		const verdict = str(form.verdict);
		if (verdict !== "pass" && verdict !== "fail")
			return c.text("verdict must be pass or fail", 400);
		const notesRaw = str(form.notes).trim();
		db.raw
			.prepare(
				`INSERT INTO audit_log (post_id, iso_week, verdict, notes, audited_at) VALUES (?, ?, ?, ?, ?)`,
			)
