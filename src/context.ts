...
export function buildContext(db: Db, def: ScraperDef): { ctx: ScraperContext; notes: string[] } {
  ...
  const notes: string[] = [];
  const counts = { documentsFetched: 0, documentsNew: 0, itemsSeen: 0, itemsNew: 0 };
  ...
  const ctx: ScraperContext = {
    ...
    async fetchDocument(url, meta) {
      ...
      return { documentId: id, body: res.body, fromCache: !isNew, contentType: res.contentType, finalUrl: res.finalUrl };
    },
    insertItem(item) {
      counts.itemsSeen++;
