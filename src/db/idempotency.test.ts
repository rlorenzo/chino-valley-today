...
function countItems(db: Db): number {
	return (
		db.raw.prepare("SELECT COUNT(*) AS n FROM items").get() as unknown as {
			n: number;
		}
	).n;
}
...
describe("item idempotency across document re-uploads", () => {
	test("a re-uploaded document updates the existing item instead of duplicating it", () => {
		...
		assert.notEqual(
			doc2,
			doc1,
			"a changed content_hash must create a distinct document",
		);
		...
		assert.equal(
			second.isNew,
			false,
			"the re-uploaded item must match the existing row",
		);
		assert.equal(
			second.id,
			first.id,
			"and must be the same row, not a new one",
		);
		assert.equal(
			countItems(db),
			1,
			"the re-upload must not duplicate the item",
		);
		...
		assert.equal(
			row.document_id,
			doc2,
			"document_id must be repointed to the newest document",
		);
	...
	test("repeated re-uploads keep collapsing onto one row", () => {
		...
		assert.equal(
			countItems(db),
			1,
			"four uploads of the same item must leave one row",
		);
	});
	...
	test("two documents from ONE source with a colliding external_id stay separate", () => {
		...
		const council = addDocument(
			db,
			sourceId,
			"a".repeat(64),
			"https://example.test/council-2026-07-14.pdf",
		);
		const planning = addDocument(
			db,
			sourceId,
			"b".repeat(64),
			"https://example.test/planning-2026-07-14.pdf",
		);
		...
		assert.equal(
			b.isNew,
			true,
			"a different document must not match, even within one source",
		);
		assert.notEqual(b.id, a.id);
		assert.equal(
			countItems(db),
			2,
			"two same-date meetings must not collapse into one item",
		);
	});
	...
	test("the same external_id from a different source stays a separate item", () => {
		...
		const chinoDoc = addDocument(
			db,
			chino,
			"a".repeat(64),
			"https://example.test/chino.pdf",
		);
		const hillsDoc = addDocument(
			db,
			hills,
			"b".repeat(64),
			"https://example.test/hills.pdf",
		);
	...
	test("a genuinely new item on a re-uploaded document is still inserted", () => {
		...
		assert.equal(
			countItems(db),
			2,
			"the added item must land alongside the carried-over one",
		);
	});
	...
	test("the match-then-write leaves no transaction open", () => {
		...
		assert.equal(
			db.raw.isTransaction,
			false,
			"the transaction must be committed, not left open",
		);
	});
	...
	test("the lookup index that serves this exists", () => {
		...
		const idx = db.raw
			.prepare(
				"SELECT name FROM sqlite_master WHERE type='index' AND name='idx_items_external_id'",
			)
			.get() as unknown as { name: string } | undefined;
		assert.ok(
			idx,
			"idx_items_external_id must exist — the UNIQUE autoindex leads with document_id",
		);
	});
});
