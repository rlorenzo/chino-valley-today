...
function isAllowed(groups: RobotGroup[], path: string): boolean {
  ...
  let applicable = groups.filter((g) => g.agents.some((a) => a !== '*' && ua.includes(a)));
  if (applicable.length === 0) applicable = groups.filter((g) => g.agents.includes('*'));
...
async function attempt(url: string, headers: Record<string, string>): Promise<Response> {
  return fetch(url, { headers, redirect: 'follow', signal: AbortSignal.timeout(60000) });
}

export async function politeFetch(url: string, opts: FetchOpts = {}): Promise<RawResult> {
  const u = new URL(url);
  if (!opts.skipRobots) {
