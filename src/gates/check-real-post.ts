...
function corpusForSources(
	...
	for (const url of sources) {
		const rows = db.raw
			.prepare(
				"SELECT title, body, meta, occurred_at FROM items WHERE source_url = ?",
			)
		...
		if (rows.length === 0) {
			...
			const docs = db.raw
				.prepare(
					"SELECT title, url, doc_type, meeting_date, location FROM documents WHERE url = ?",
				)
				.all(url) as unknown as {
				title: string | null;
				url: string;
			...
			for (const d of docs)
				parts.push(
					[d.title, d.doc_type, d.meeting_date, d.location]
						.filter(Boolean)
						.join(" "),
				);
			continue;
		...
		for (const r of rows) {
			parts.push(
				[r.title, r.body, r.meta, r.occurred_at].filter(Boolean).join(" "),
			);
		}
	}
	return parts.join("\n\n");
...
function main(): void {
	...
	for (const relPath of files) {
		...
		console.log(`\n=== ${relPath} — "${title}" ===`);
		console.log(
			`sources: ${sources.length}, corpus chars: ${inputCorpus.length}`,
		);
		...
		if (!report.pass) {
			for (const f of report.failures) {
				console.log(
					`  [${f.gate}] ${f.detail}${f.excerpt ? ` — "${f.excerpt}"` : ""}`,
				);
			}
		}
	}
