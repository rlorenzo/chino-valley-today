...
describe("Gate 1a — citations", () => {
	test("fails: substantive paragraph with no link", () => {
		const input: GateInput = {
			...
			inputCorpus:
				"The council approved the new budget after a lengthy discussion.",
		};
		const report = validateDraft(input);
	...
	test("fails: link URL not in allowedUrls (exact-match, no prefix match)", () => {
		const input: GateInput = {
			...
			allowedUrls: [
				"https://chino.legistar.com/MeetingDetail.aspx?ID=1&GUID=X",
		...
		assert.ok(
			cite.some((f) => f.detail.includes("not in the allowed source list")),
		);
	});
	...
	test("a mid-document --- that is NOT the disclosure footer does not exempt what follows it (fail-closed)", () => {
		const input: GateInput = {
			...
			allowedUrls: ["https://example.com/a"],
			inputCorpus:
				"The council approved the item. The council also approved a second item.",
		};
		const report = validateDraft(input);
		assert.equal(report.pass, false);
		assert.ok(failuresFor(report.failures, "citations").length > 0);
	...
	test("fails: empty draft", () => {
		...
		assert.equal(report.pass, false);
		assert.ok(
			failuresFor(report.failures, "citations").some((f) =>
				f.detail.includes("no substantive content"),
			),
		);
	});
...
describe("Gate 1b — numeric consistency", () => {
	test("fails: invented dollar amount not in corpus", () => {
		...
		assert.ok(
			numeric.some(
				(f) => f.detail.includes("99,000") || f.detail.includes("99000"),
			),
		);
	});
	...
	test("fails: vote tally not corroborated by corpus", () => {
		...
		assert.equal(report.pass, false);
		assert.ok(
			failuresFor(report.failures, "numeric").some((f) =>
				f.detail.includes("vote tally"),
			),
		);
	});
...
describe("Gate 1c — proper-name whitelist", () => {
	test("fails: invented person name not in corpus", () => {
		const input: GateInput = {
			...
			allowedUrls: ["https://example.com/a"],
			inputCorpus:
				"Eunice M. Ulloa moved to approve the item. The motion carried.",
		};
		const report = validateDraft(input);
		assert.equal(report.pass, false);
		assert.ok(
			failuresFor(report.failures, "proper_names").some((f) =>
				f.detail.includes("Jonathan Reyes"),
			),
		);
	});
	...
	test("fails: garbled name variant (transcript-typo name) even though a similar real name exists in corpus", () => {
		const input: GateInput = {
			...
			allowedUrls: ["https://example.com/a"],
			inputCorpus:
				"Maria Lopez seconded the motion during the regular meeting.",
		};
		const report = validateDraft(input);
		assert.equal(report.pass, false);
		assert.ok(
			failuresFor(report.failures, "proper_names").some((f) =>
				f.detail.includes("Marria Lopez"),
			),
		);
	});
	...
	test("fails: hallucinated composite name split across a mid-sentence link boundary (Codex review finding)", () => {
		...
		const input: GateInput = {
			...
			allowedUrls: ["https://example.com/a"],
			inputCorpus:
				"Maria attended the workshop. Councilmember Lopez was absent.",
		};
		const report = validateDraft(input);
		assert.equal(report.pass, false);
		assert.ok(
	...
	test("fails: composite-name bypass via a decoy second link before a comma (Codex follow-up finding)", () => {
		...
		const input: GateInput = {
			...
			allowedUrls: ["https://example.com/a"],
			inputCorpus:
				"Maria attended the workshop. Councilmember Lopez was absent. Source materials were provided.",
		};
		const report = validateDraft(input);
		assert.equal(report.pass, false);
		assert.ok(
	...
	test("fails: garbled name inside a link label is still scanned and caught", () => {
		const input: GateInput = {
			...
			allowedUrls: ["https://example.com/a"],
			inputCorpus:
				"The board heard the report from Elise Buckley during the meeting.",
		};
		const report = validateDraft(input);
		assert.equal(report.pass, false);
		assert.ok(
			failuresFor(report.failures, "proper_names").some((f) =>
				f.detail.includes("Elise Jukley"),
			),
		);
	});
