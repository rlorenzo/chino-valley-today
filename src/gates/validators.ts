...
type BlockKind = 'heading' | 'hr' | 'footer' | 'list-fact' | 'list-pure-link' | 'paragraph';
...
function parseBlocks(bodyMd: string): Block[] {
  ...
  for (const chunk of chunks) {
    ...
    if (LIST_ITEM_RE.test(lines[0])) {
      ...
      for (const item of items) {
        const t = item.trim();
        if (t === '') continue;
        blocks.push({ kind: PURE_LINK_ITEM_RE.test(t) ? 'list-pure-link' : 'list-fact', raw: t });
      }
      continue;
    }
  ...
  let lastHr = -1;
  for (let i = 0; i < blocks.length; i++) if (blocks[i].kind === 'hr') lastHr = i;
  if (lastHr !== -1) {
    ...
    if (FOOTER_MARKER_RE.test(trailing)) {
      for (let i = lastHr; i < blocks.length; i++) blocks[i] = { ...blocks[i], kind: 'footer' };
    }
  }
...
function runCitationsGate(
  ...
  const substantive = blocks.filter((b) => b.kind === 'paragraph' || b.kind === 'list-fact');
  const scannable = blocks.filter((b) => b.kind !== 'footer');

  if (substantive.length === 0) {
    failures.push({
      gate: 'citations',
      detail: 'draft has no substantive content (no paragraphs or fact-bearing list items found)',
    });
  }
...
const MONTH_ALT = Object.keys(MONTH_INDEX).concat(Object.keys(MONTH_ABBR)).join('|');
...
function extractDates(text: string): DateMatch[] {
  ...
  for (const m of text.matchAll(/(?<!\d)(\d{4})-(\d{1,2})-(\d{1,2})(?!\d)/g)) {
    ...
    if (mo >= 1 && mo <= 12 && d >= 1 && d <= 31) {
      out.push({ start: m.index, end: m.index + m[0].length, key: `${y}-${pad2(mo)}-${pad2(d)}`, raw: m[0] });
    }
  }
  ...
  const monthDayYearRe = new RegExp(`\\b(${MONTH_ALT})\\.?\\s+(\\d{1,2})(?:st|nd|rd|th)?,?\\s+(\\d{4})(?!\\d)`, 'gi');
  for (const m of text.matchAll(monthDayYearRe)) {
    const mo = monthLookup(m[1]);
    const d = Number(m[2]);
    const y = Number(m[3]);
    if (mo !== undefined && d >= 1 && d <= 31) {
      out.push({ start: m.index, end: m.index + m[0].length, key: `${y}-${pad2(mo + 1)}-${pad2(d)}`, raw: m[0] });
    }
  }

  const dayMonthYearRe = new RegExp(`(?<!\\d)(\\d{1,2})(?:st|nd|rd|th)?\\s+(${MONTH_ALT})\\.?,?\\s+(\\d{4})(?!\\d)`, 'gi');
...
function extractTimes(text: string): TimeMatch[] {
  ...
  for (const m of text.matchAll(re)) {
    ...
    const mm = m[2];
    if (h >= 0 && h <= 23) {
      out.push({ start: m.index, end: m.index + m[0].length, key: `${h}:${mm}`, raw: m[0] });
    }
  }
  return out;
}
...
function extractGenericNumbers(text: string): NumMatch[] {
  ...
  for (const m of text.matchAll(NUM_RE)) {
    ...
    if (value !== null) out.push({ start: m.index, end: m.index + m[0].length, value, raw: m[0] });
  }
  return out;
}

function normalizeNumberForCompare(raw: string): number | null {
  const cleaned = raw.replace(/percent|per\s?cent/gi, '').replace(/[$,%]/g, '').trim();
  if (cleaned === '' || cleaned === '.') return null;
  const n = Number(cleaned);
  return Number.isFinite(n) ? n : null;
}

function runNumericGate(scanText: string, inputCorpus: string): { failures: GateFailure[]; stats: Record<string, number> } {
  const failures: GateFailure[] = [];
  ...
  for (const d of draftDates) {
    ...
    if (!corpusDateKeys.has(d.key)) {
      ...
      failures.push({
        ...
        excerpt: truncate(scanText.slice(Math.max(0, d.start - 40), d.end + 40)),
      });
    }
  }

  const draftTimes = extractTimes(scanText).filter((t) => !overlaps(t, consumed));
  ...
  for (const t of draftTimes) {
    ...
    if (!corpusTimeKeys.has(t.key)) {
      ...
      failures.push({
        ...
        excerpt: truncate(scanText.slice(Math.max(0, t.start - 40), t.end + 40)),
      });
    }
  }

  const draftTallies = extractTallies(scanText).filter((t) => !overlaps(t, consumed));
  const corpusNumberSet = new Set(extractGenericNumbers(inputCorpus).map((n) => n.value));
  let talliesFailed = 0;
  ...
  const draftNumbers = extractGenericNumbers(scanText).filter((n) => !overlaps(n, consumed));
  let numbersFailed = 0;
  for (const n of draftNumbers) {
    if (!corpusNumberSet.has(n.value)) {
      ...
      failures.push({
        ...
        excerpt: truncate(scanText.slice(Math.max(0, n.start - 40), n.end + 40)),
      });
    }
  }
...
const MONTH_ABBR_DISPLAY = ['jan', 'feb', 'mar', 'apr', 'jun', 'jul', 'aug', 'sep', 'sept', 'oct', 'nov', 'dec'];
const US_STATES = [
...
const BUILTIN_ALLOWLIST_SET = new Set(BUILTIN_ALLOWLIST.map((s) => s.toLowerCase()));
...
export const SENTENCE_INITIAL_COMMON_WORDS: readonly string[] = [
  ...
  'if', 'when', 'while', 'after', 'before', 'during', 'on', 'in', 'at', 'according', 'under',
  'following', 'because', 'although', 'though', 'but', 'and', 'or', 'so', 'yet', 'for', 'with',
  'without', 'within', 'once', 'as', 'since', 'until', 'unless', 'meanwhile', 'however',
  'therefore', 'additionally', 'also', 'council', 'commission', 'board', 'committee', 'members',
  'member', 'councilmembers', 'trustees', 'staff', 'residents', 'officials', 'voters',
  'attendees', 'meeting', 'item', 'agenda', 'motion', 'vote', 'public', 'city', 'district',
  'department', 'police', 'fire', 'date', 'time', 'location', 'status', 'type', 'source',
];
const SENTENCE_INITIAL_SET = new Set(SENTENCE_INITIAL_COMMON_WORDS.map((s) => s.toLowerCase()));
...
export const TITLE_WORDS: readonly string[] = [
  'mayor', 'councilmember', 'councilman', 'councilwoman', 'superintendent', 'officer',
  'detective', 'deputy', 'sergeant', 'captain', 'chief', 'commissioner', 'trustee',
  'president', 'chair', 'vicechair', 'judge', 'sheriff', 'dr', 'mr', 'mrs', 'ms',
  'principal', 'director', 'administrator', 'lieutenant', 'governor', 'senator',
...
const CONNECTORS = new Set(['of', 'de', 'la', 'del', 'da', 'van', 'von', 'der']);
...
function tokenizeBySentence(text: string): Token[][] {
  return splitSentences(text).map(({ text: part, start }) => {
    ...
    for (const m of part.matchAll(WORD_RE)) {
      tokens.push({ text: m[0], start: start + m.index, end: start + m.index + m[0].length, sentenceInitial: first });
      first = false;
    }
    return tokens;
  });
...
function stripTitleAndLead(tokens: string[], sentenceInitial: boolean): string[] | null {
  let t = tokens;
  let leadIsSentenceInitial = sentenceInitial;
  if (leadIsSentenceInitial && t.length > 0 && SENTENCE_INITIAL_SET.has(t[0].toLowerCase())) {
    t = t.slice(1);
  ...
  if (t.length > 0 && TITLE_WORDS_SET.has(t[0].toLowerCase())) {
    t = t.slice(1);
  } else if (t.length > 1 && t[0].toLowerCase() === 'council' && t[1].toLowerCase() === 'member') {
    t = t.slice(2);
  }
...
function findNameSequences(text: string): { phrase: string; sentenceInitial: boolean; start: number; end: number }[] {
  const out: { phrase: string; sentenceInitial: boolean; start: number; end: number }[] = [];
  for (const tokens of tokenizeBySentence(text)) {
    ...
    while (i < tokens.length) {
      ...
      while (j < tokens.length) {
        ...
        if (/^[A-Z]/.test(tokens[j].text) && isWhitespaceGap(text, prevEnd, tokens[j].start)) {
          seq.push(tokens[j]);
          j++;
        } else if (
...
function runProperNamesGate(
  ...
  const corpusNorm = collapseWhitespace(inputCorpus.replace(/\./g, ' ')).toLowerCase();
  ...
  for (const seq of sequences) {
    ...
    failures.push({
      ...
      excerpt: truncate(scanText.slice(Math.max(0, seq.start - 40), seq.end + 40)),
    });
  }
...
export function validateDraft(input: GateInput): GateReport {
  ...
  const scanBlocks = blocks.filter((b) => b.kind === 'paragraph' || b.kind === 'list-fact');
  ...
  const failures = [...citations.failures, ...numeric.failures, ...names.failures];
