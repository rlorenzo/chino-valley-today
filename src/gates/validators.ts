...
const SENTENCE_INITIAL_COMMON_WORDS: readonly string[] = [
	...
	"additionally",
	"also",
	// Determiners, quantifiers, numerals, possessives and place/time adverbs.
	// These are the words a generator reaches for to open a sentence, and the
	// reason they flag is that the generator supplies them and the source does
	// not: "Another extreme heat warning" over a corpus that says "An extreme
	// heat warning" held the 2026-W38 podcast, because a podcast turn makes
	// every sentence sentence-initial. A content word in that slot ("Expected
	// highs...", "Severe weather...") is quoted from the corpus and grounds
	// itself, so only this closed function-word class needs listing — and it
	// is listed to completion deliberately. A 'missing' word here is a held
	// episode; a spurious one costs nothing, since nobody is named "Neither"
	// and stripping the lead still checks whatever follows it.
	"another",
	"each",
	"every",
	"all",
