...
// Inference catalog.
import { apiKeyFor, LLM_TASKS } from "./config.ts";
...
if (!res.ok) {
	console.error(
		`GET ${endpoint}/models -> HTTP ${res.status}: ${(await res.text()).slice(0, 300)}`,
	);
	process.exit(1);
}
...
for (const [task, cfg] of Object.entries(LLM_TASKS)) {
	...
	if (!found) ok = false;
	console.log(
		`${found ? "OK  " : "MISS"} ${task}: ${cfg.model}${found ? "" : "  <-- not in catalog; set CVT_MODEL_* in .env"}`,
	);
}
process.exit(ok ? 0 : 2);
