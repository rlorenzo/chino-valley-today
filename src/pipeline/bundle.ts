...
export function assembleBundle(
	...
	const agendaItems = shape.agenda
		? itemsFor(db, shape.agenda.key, shape.agenda.type, isoDate)
		: [];
	...
	if (typeof metaBody === "string" && metaBody.trim()) {
		...
		bodyName =
			shape.cityPrefix &&
			!name.toLowerCase().includes(shape.cityPrefix.toLowerCase())
...
export function renderBundleForPrompt(
	b: MeetingBundle,
	...
	if (b.agendaItems.length) {
		...
		for (const it of b.agendaItems) {
			lines.push(
				`- ${it.title ?? "(untitled)"}${it.meta.agendaNumber ? ` [item ${it.meta.agendaNumber}]` : ""}`,
			);
			...
			if (action !== undefined && action !== null)
				lines.push(`  action: ${String(action)}`);
		}
	}
	if (b.votes.length) {
	...
	if (b.transcriptSegments.length) {
		lines.push(
			"",
			"## Transcript (machine-generated; names may be misrecognized — never introduce a name that appears ONLY here):",
		);
...
function weekItemsFor(
	...
	return rows
		.filter((r) => isoWeekOfOccurredAt(r.occurred_at) === isoWeek)
		.map(toBundleItem);
}
...
export function listBusinessWeeks(db: Db): Array<{
	...
	const counts = new Map<
		string,
		{ licenseEvents: number; planningItems: number }
	>();
	const bump = (
		week: string | null,
		field: "licenseEvents" | "planningItems",
	) => {
		if (!week) return;
	...
	for (const r of all(BUSINESS_SOURCES.licenses))
		bump(isoWeekOfOccurredAt(r.occurred_at), "licenseEvents");
	for (const r of all(BUSINESS_SOURCES.planning)) {
		if (isBusinessRelevant(r.title, r.body))
			bump(isoWeekOfOccurredAt(r.occurred_at), "planningItems");
	}
...
export function licenseEventDetail(
	meta: Record<string, unknown>,
): string | null {
	...
	if (licenseNo)
		parts.push(
			`License ${licenseNo}${licenseType ? ` (Type ${licenseType})` : ""}.`,
		);
	...
	const dba = s("dba");
	if (primary && dba)
		parts.push(`Licensee: ${primary}, doing business as ${dba}.`);
...
export function assembleBusinessBundle(
	db: Db,
	isoWeek: string,
): BusinessBundle | null {
	...
	const planningItems = weekItemsFor(
		...
		BUSINESS_SOURCES.planning.type,
		isoWeek,
