...
import { isoWeekOfOccurredAt, isBusinessRelevant, licenseEventDetail } from './bundle.ts';
...
describe('isBusinessRelevant', () => {
  test('matches the business-relevant patterns in titles', () => {
    assert.ok(
      isBusinessRelevant('PL25-0085 (Special Conditional Use Permit), PL25-0086 (Site Approval) and Development Agreement', null)
    );
    assert.ok(isBusinessRelevant('Business License Reform and Small Business Protection Measure​.', null));
    assert.ok(isBusinessRelevant('Zoning Code Amendment for industrial parcels', null));
    assert.ok(isBusinessRelevant('Zone Change 2026-001', null));
    assert.ok(isBusinessRelevant('Rezoning of the northeast corner parcel', null));
    assert.ok(isBusinessRelevant('Massage establishment licensing update', null));
  });

  test('matches in the body when the title is silent', () => {
    assert.ok(isBusinessRelevant('Public hearing', 'Consider a conditional use permit for a drive-through.'));
  });

  test('does not match unrelated council business', () => {
    assert.equal(isBusinessRelevant('Approval of Meeting Minutes', null), false);
    assert.equal(isBusinessRelevant('Proclamation Honoring Volunteer of the Year', null), false);
    assert.equal(isBusinessRelevant('Award of a landscape maintenance contract', 'Ayala Park irrigation.'), false);
  ...
  test('near-miss phrasings do not match — the patterns are anchored and specific', () => {
    ...
    assert.equal(isBusinessRelevant('Encroachment permit for sidewalk repair', null), false);
    assert.equal(isBusinessRelevant('Temporary use permit for a community carnival', null), false);
    assert.equal(isBusinessRelevant('Professional services agreement for irrigation design', null), false);
    assert.equal(isBusinessRelevant('Licorice vendor booth at the community fair', null), false);
    assert.equal(isBusinessRelevant('Discussion of unlicensed street vending', null), false);
    assert.equal(isBusinessRelevant('Establish a Capital Project for storm drain repairs', null), false);
  });
});
...
describe('licenseEventDetail', () => {
  const META = {
    ...
    attempted_detail_url: 'https://www.abc.ca.gov/licensing/license-lookup/single-license/?RPTTYPE=12&LICENSE=399692',
  };
  ...
  test('empty or fact-free meta returns null', () => {
    ...
    assert.equal(licenseEventDetail({ attempted_detail_url: 'https://x.example' }), null);
  });
});
