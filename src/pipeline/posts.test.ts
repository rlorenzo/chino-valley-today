...
import { glossaryFor, renderPostFile, type NewPost } from './posts.ts';
import { validateDraft } from '../gates/validators.ts';

const SOURCE = 'https://www.abc.ca.gov/licensing/licensing-reports/status-changes/';

function post(bodyMd: string): NewPost {
  return { slug: 's', postType: 'business_tracker', tier: 'B', title: 't', bodyMd, sources: [SOURCE] };
}
...
describe('record-code glossary', () => {
  test('includes only the codes the post actually uses', () => {
    const g = glossaryFor('License 399692, Type 20, changed from ACTIVE to REVPEN per ABC.');
    assert.match(g, /ABC —/);
    assert.match(g, /Type 20 —/);
    assert.match(g, /ACTIVE —/);
    assert.match(g, /REVPEN —/);
    assert.doesNotMatch(g, /Type 41/, 'a code the post never mentions must not be glossed');
...
describe('glossary placement in the rendered post', () => {
  test('sits after the final hr, with the disclosure line still in that region', () => {
    const file = renderPostFile(post('Type 41 licensee moved ACTIVE to REVPEN.'), '2026-08-15T00:00:00Z');
    const lastHr = file.lastIndexOf('\n---\n');
    const trailing = file.slice(lastHr);
    assert.match(trailing, /What the record codes mean/, 'glossary must be inside the footer region');
    assert.match(trailing, /Generated from public records/, 'the footer marker must remain in that region');
  });
  ...
  test('introduces no horizontal rule of its own', () => {
    ...
    const file = renderPostFile(post('Type 41 ACTIVE to REVPEN.'), '2026-08-15T00:00:00Z');
    const body = gateBody(file);
    assert.equal(body.match(/^---$/gm)?.length ?? 0, 1, 'exactly one hr, the footer separator');
  });

  test('a post with no codes renders the plain footer unchanged', () => {
    const file = renderPostFile(post('The council approved the contract.'), '2026-08-15T00:00:00Z');
...
describe('the glossary is invisible to the gates', () => {
  ...
  test('adding it changes no gate outcome and no gate statistic', () => {
    ...
    const withoutGlossary = validateDraft({ bodyMd: body, allowedUrls: [SOURCE], inputCorpus: corpus });

    assert.equal(withoutGlossary.pass, true, 'precondition: the bare body passes');
    assert.equal(withGlossary.pass, true, 'the glossary must not hold the draft');
    ...
    const { blocksTotal: totalWithout, ...scannedWithout } = withoutGlossary.stats;
    assert.deepEqual(scannedWith, scannedWithout, 'the glossary must be scanned by nothing');
    assert.ok(totalWith > totalWithout, 'the glossary blocks are parsed, just classified as footer');
  });
  ...
  test('no glossary term is ever reported as an unknown name', () => {
    ...
    const names = r.failures.filter((f) => f.gate === 'proper_names').map((f) => f.detail);
    assert.equal(
