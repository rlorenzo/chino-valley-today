...
function headerIndex($: cheerio.CheerioAPI, table: cheerio.Cheerio<AnyNode>): Map<string, number> {
...
function cellText($: cheerio.CheerioAPI, cells: cheerio.Cheerio<AnyNode>, idx: number | undefined): string {
...
function cellLines($: cheerio.CheerioAPI, cells: cheerio.Cheerio<AnyNode>, idx: number | undefined): string[] {
  if (idx === undefined) return [];
  const html = cells.eq(idx).html() ?? '';
  return html
    .split(/<br\s*\/?>/i)
    .map((frag) => cheerio.load(`<div>${frag}</div>`)('div').text().replace(/\s+/g, ' ').trim())
    .filter((s) => s.length > 0);
}
...
function premisesAddress(c: CommonFields): string {
  const parts = [c.premisesStreet, c.city ? `${c.city}, CA ${c.zip}`.trim() : ''].filter(Boolean);
  return parts.join(', ');
}
...
async function ingestNewApplications(ctx: ScraperContext): Promise<{ matched: number; total: number }> {
  ...
  if (!reportDate) {
    ctx.note('Could not parse a report date from #rptdate on new-applications; items below use null occurred_at.');
  }
  ...
  table.find('tbody > tr').each((_, tr) => {
    ...
    total++;
    const common = parseCommon($, cells, cols, 'primary owner and premises addr.');
    ...
    ctx.insertItem({
      ...
      title: `${common.primaryName || common.dba || 'Unknown applicant'} — Type ${common.licenseType} ${status}`.trim(),
      body: null,
...
async function ingestStatusChanges(ctx: ScraperContext): Promise<{ matched: number; total: number }> {
  ...
  table.find('tbody > tr').each((_, tr) => {
    ...
    ctx.insertItem({
      ...
      title: `${common.primaryName || common.dba || 'Unknown owner'} — Type ${common.licenseType} ${statusFrom}→${statusTo}`.trim(),
      body: null,
      occurred_at: reportDate,
      meta: {
        ...
        district_code: common.districtCode,
        geo_code: common.geoCode,
        original_issue_date: mmddyyyyToIso(issueDateRaw) ?? (issueDateRaw || null),
        expiry_date: common.expiryDateIso ?? (common.expiryDateRaw || null),
...
const scraper: ScraperDef = {
  ...
  async run(ctx) {
    ...
    ctx.note('Response format: server-rendered HTML (a DataTables-driven <table>), not JSON or CSV — despite the plugin enqueuing DataTables\' CSV/PDF export buttons client-side, no JSON/CSV endpoint was found or needed; the DOM already carries every column DataTables would export, including the hidden ones.');
  },
};
