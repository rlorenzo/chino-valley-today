...
function extractRowsForCategory(
	$: cheerio.CheerioAPI,
	cat: CategoryInfo,
): AgendaRow[] {
	...
	section.find("tr.catAgendaRow, tbody tr").each((_, tr) => {
		...
		if (!href) return;
		const title =
			pdfLink.text().replace(/\s+/g, " ").trim() || `${cat.name} agenda`;
		rows.push({
			category: cat,
			meetingDate,
			title,
...
async function ingestAgenda(
	...
	ctx.note(
		`Split "${row.title}" into ${items.length} agenda_item row(s) (item numbers: ${items.map((i) => i.n).join(", ")}).`,
	);
}
...
const scraper: ScraperDef = {
	...
	async run(ctx) {
		...
		if (!listing.ok)
			throw new Error(`HTTP ${listing.status} fetching ${AGENDA_CENTER_URL}`);
		...
		const rssAll = await ctx.fetchRaw(
			`${BASE}/RSSFeed.aspx?ModID=65&CID=All-0`,
		);
		const rssItemCount = (rssAll.body.toString("utf8").match(/<item>/g) ?? [])
			.length;
		ctx.note(
		...
		for (const cat of categories) {
			...
			ctx.note(
				`Category "${cat.name}": ${catRows.length} agenda PDF(s) in its initially-rendered ("current") year.`,
			);
