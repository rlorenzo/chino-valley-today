...
const RTF_IGNORABLE_DESTINATIONS = new Set([
  ...
  'themedata', 'colorschememapping', 'latentstyles', 'rsid', 'listtable',
  'listoverridetable', 'panose', 'falt', 'nonshppict', 'shpinst',
]);
      ...
      if (/[a-zA-Z]/.test(c2)) {
        ...
        if (c2 === "'") {
          ...
          if (!skipping) {
            ...
            if (!Number.isNaN(code)) out += Buffer.from([code]).toString('latin1');
          }
          continue;
        } else {
          if (!skipping) out += c2;
...
async function fetchLegislationLinks(ctx: ScraperContext, event: LegistarEvent): Promise<Map<string, string>> {
  ...
  try {
    ...
    $('a[id*="hypFile"]').each((_i, a) => {
      ...
      if (file && href) map.set(file, new URL(href, event.EventInSiteURL).toString());
    });
  } catch (err) {
    ctx.note(
...
function itemSourceUrl(item: LegistarEventItem, legislationLinks: Map<string, string>, fallback: string): string {
...
function itemBody(item: LegistarEventItem): string | null {
  const parts = [rtfToText(item.EventItemAgendaNote), rtfToText(item.EventItemMinutesNote)].filter(
    (t): t is string => !!t
  );
...
async function fetchEventItems(
  ...
  const doc = await ctx.fetchDocument(`${API}/events/${event.EventId}/eventitems?AgendaNote=1&MinutesNote=1`, {
    docType: 'agenda',
...
async function ingestMeeting(
  ...
  if (event.EventAgendaFile) {
    ...
    try {
      ...
      ctx.note(`Failed to fetch agenda PDF for EventId ${event.EventId}: ${(err as Error).message}`);
    }
  }
}
...
async function sampleVotes(
  ...
  for (const item of sample) {
    ...
    try {
      const res = await ctx.fetchRaw(`${API}/eventitems/${item.EventItemId}/votes`);
      if (!res.ok) {
        ctx.note(`Votes probe for EventItemId ${item.EventItemId} returned HTTP ${res.status}.`);
        continue;
      }
      votes = JSON.parse(res.body.toString('utf8')) as LegistarVote[];
    } catch (err) {
      ctx.note(`Votes probe failed for EventItemId ${item.EventItemId}: ${(err as Error).message}`);
      continue;
    }
    if (votes.length === 0) continue;
    const selfConsistent = votes.every((v) => v.VoteEventItemId === item.EventItemId);
    if (!selfConsistent) {
      mismatched++;
      continue;
    }
    consistent++;
    const sourceUrl = itemSourceUrl(item, legislationLinks, event.EventInSiteURL);
    const occurredAt = new Date(event.EventDate).toISOString();
...
const scraper: ScraperDef = {
  ...
  async run(ctx) {
    ...
    const eventsDoc = await ctx.fetchDocument(`${API}/events?%24top=30&%24orderby=EventDate%20desc`, {
      docType: 'listing',
      title: 'Chino Legistar recent events (top 30, EventDate desc)',
    });
    const events = JSON.parse(eventsDoc.body.toString('utf8')) as LegistarEvent[];
    const bodies = [...new Set(events.map((e) => e.EventBodyName.trim()))];
    ctx.note(`Fetched ${events.length} recent events. EventBodyName values seen: ${bodies.join(', ')}.`);
    ...
    const agendaStatuses = [...new Set(events.map((e) => e.EventAgendaStatusName))];
    ctx.note(
    ...
    const isPublishedAgenda = (e: LegistarEvent) =>
      e.EventAgendaStatusName === 'Final' || e.EventAgendaStatusName === 'Final Revised';
    const isCancelled = (e: LegistarEvent) => (e.EventComment ?? '').includes('Cancelled');
    ...
    const councilCandidates = events.filter(
      (e) => e.EventBodyName.trim() === 'City Council' && isPast(e) && isPublishedAgenda(e)
    );
    ...
    await ingestMeeting(ctx, targetMeeting, targetDocId, targetItems, targetLinks);
    await sampleVotes(ctx, targetMeeting, targetDocId, targetItems, targetLinks);

    const withMatter = targetItems.filter((i) => i.EventItemMatterFile);
    const withRealLink = withMatter.filter((i) => targetLinks.has(i.EventItemMatterFile as string));
    ...
    const priorityOrder = ['Planning Commission', ...[...otherCandidatesByBody.keys()].filter((b) => b !== 'Planning Commission')];
    ...
    for (const body of priorityOrder) {
      ...
      const { documentId, items } = await fetchEventItems(ctx, cand);
      if (items.length === 0) {
        ctx.note(`${body} EventId ${cand.EventId} (${cand.EventDate.slice(0, 10)}): 0 event items — skipping.`);
        continue;
      }
      ctx.note(`Probe meeting: ${body} EventId=${cand.EventId}, date=${cand.EventDate.slice(0, 10)}, ${items.length} event items.`);
      const probeLinks = await fetchLegislationLinks(ctx, cand);
      await ingestMeeting(ctx, cand, documentId, items, probeLinks);
      probed++;
    }
    if (probed === 0) {
      ctx.note('No non-City-Council body had a past, published, non-cancelled meeting with items in the top-30 window to probe.');
    }
    ...
    try {
      ...
      if (mattersProbe.ok) {
        const matters = JSON.parse(mattersProbe.body.toString('utf8')) as Array<{
          MatterName?: string;
          MatterTitle?: string;
          MatterFile?: string;
        }>;
        const testLooking = matters.filter(
          (m) => /\btest\b/i.test(m.MatterTitle ?? '') || /\bsample\b/i.test(m.MatterName ?? '')
        );
        ctx.note(
    ...
    try {
      ...
      ctx.note(
        `RSS fallback probe: ${INSITE}/Feed.ashx?M=Calendar -> HTTP ${feedProbe.status}` +
          (feedProbe.ok ? ' (feed exists; not ingested — Web API is the primary, richer path for this source).' : '.')
      );
    } catch (err) {
