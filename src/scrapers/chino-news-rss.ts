...
function laOffsetMinutes(approxUtcMs: number): number {
  ...
  const m = (parts.find((p) => p.type === 'timeZoneName')?.value ?? 'GMT-08:00').match(/GMT([+-])(\d{2}):(\d{2})/);
  if (!m) return -480;
...
function parseRssItems(xml: string): FeedItem[] {
  ...
  return items.map((it) => {
    ...
    const guid = (typeof guidField === 'object' ? guidField?.['#text'] : guidField) ?? String(it.link ?? '');
    const extra: Record<string, string> = {};
    for (const k of Object.keys(it)) {
      if (k.startsWith('calendarEvent:')) extra[k.slice('calendarEvent:'.length)] = String(it[k]);
    }
    return {
...
const NEWSFLASH_CATEGORIES = [
  ...
  { cid: 'Community-Services-Spotlights-7', name: 'Community Services Spotlights' },
...
function extractNewsflashDetail($: cheerio.CheerioAPI): { text: string; selector: string } {
  const primary = $('#main-wrapper .article-content').text().replace(/\s+/g, ' ').trim();
  if (primary) return { text: primary, selector: '#main-wrapper .article-content' };
  const fallback = $('#moduleContent').text().replace(/\s+/g, ' ').trim();
  return { text: fallback, selector: '#moduleContent (fallback — generic Pages-module layout, lower confidence)' };
}
...
async function run(ctx: ScraperContext): Promise<void> {
  ...
  const allDoc = await ctx.fetchDocument(allFeedUrl, { docType: 'feed', title: 'Newsflash — All categories' });
  const allItems = parseRssItems(allDoc.body.toString('utf8'));
  ctx.note(`Newsflash/CivicAlerts "All" feed (ModID=1, aggregate) currently has ${allItems.length} item(s) site-wide: ${allItems.map((i) => `"${i.title}"`).join(', ') || '(none)'}.`);
  ...
  const newsflashCollected: Array<{ item: FeedItem; category: string; feedUrl: string; documentId: number }> = [];
  for (const cat of NEWSFLASH_CATEGORIES) {
    const url = `${BASE}/RSSFeed.aspx?ModID=1&CID=${cat.cid}`;
    const doc = await ctx.fetchDocument(url, { docType: 'feed', title: `Newsflash — ${cat.name}` });
    const items = parseRssItems(doc.body.toString('utf8'));
    if (items.length === 0) {
      ctx.note(`Newsflash category "${cat.name}" (CID=${cat.cid}) currently has 0 items.`);
      continue;
    }
    for (const it of items) {
      newsflashCollected.push({ item: it, category: cat.name, feedUrl: url, documentId: doc.documentId });
    }
  }

  // Task 0.9 (Chino PD half): does PD content flow through the CivicAlerts feed?
  const pdSpotlights = newsflashCollected.filter((c) => c.category === 'Police Spotlights');
  if (pdSpotlights.length > 0) {
    ctx.note(
      `Task 0.9: Chino PD content DOES flow through the CivicAlerts/Newsflash feed via a dedicated "Police Spotlights" category (ModID=1, CID=Police-Spotlights-8). Evidence: ${pdSpotlights
        .map((c) => `"${c.item.title}"`)
        .join(', ')}. Content observed is community-outreach/recruiting (academy, events), not incident/crime press releases — Chino PD does not appear to publish incident reports through this channel.`
    );
  } else {
    ctx.note('Task 0.9: the "Police Spotlights" CivicAlerts category (ModID=1, CID=Police-Spotlights-8) exists but had 0 items at run time.');
  }
  ...
  const pdCalDoc = await ctx.fetchDocument(pdCalUrl, { docType: 'feed', title: 'Calendar — Police Department' });
  ...
  const detailTargetGuids = new Set(newsflashCollected.slice(0, 2).map((c) => c.item.guid));
  const fullTextByGuid = new Map<string, { documentId: number; sourceUrl: string; text: string; selector: string }>();
  for (const c of newsflashCollected) {
    if (!detailTargetGuids.has(c.item.guid)) continue;
    const detailDoc = await ctx.fetchDocument(c.item.link, { docType: 'news_release', title: c.item.title });
    ...
    if (fullText.length > 0) {
      fullTextByGuid.set(c.item.guid, { documentId: detailDoc.documentId, sourceUrl: c.item.link, text: fullText, selector });
    }
  }
  ctx.note(
  ...
  for (const c of newsflashCollected) {
    ...
    ctx.insertItem({
      document_id: resolveDocumentId(ctx, freshDocId, c.item.guid, 'news_release'),
      ...
      meta: {
        ...
        ...(enriched ? { fullTextExtractedVia: enriched.selector } : { bodyIsTeaser: true }),
      },
    });
  }
  ...
  const nrListDoc = await ctx.fetchDocument(nrListUrl, { docType: 'listing', title: 'News Releases (NR series) index' });
  const $list = cheerio.load(nrListDoc.body.toString('utf8'));
  const nrEntries = $list('a[href*="conta.cc"]')
    .toArray()
    .map((el) => ({ url: $list(el).attr('href') ?? '', text: $list(el).text().trim() }))
    .filter((e) => e.url)
    .map((e) => {
      const m = e.text.match(/^(NR\d{2}-\d+)\s*-\s*(.+)$/s);
      return m ? { url: e.url, nrNumber: m[1], title: m[2] } : { url: e.url, nrNumber: null, title: e.text };
    })
    .filter((e): e is { url: string; nrNumber: string; title: string } => e.nrNumber !== null);
  ctx.note(
  ...
  for (const nr of nrToIngest) {
    ...
    ctx.note(
      `NR release ${nr.nrNumber}: fetched ${nr.url} -> archived ${relDoc.finalUrl} (${bodyText.length} chars after stripping the Constant Contact email chrome at the "Unsubscribe" marker). ${
        occurredAt ? `Dateline parsed from body: ${dateline![1]}.` : 'No dateline found in extracted text — occurred_at left null.'
      }`
    );
    ctx.insertItem({
      document_id: resolveDocumentId(ctx, relDoc.documentId, nr.nrNumber, 'news_release'),
      source_url: nr.url,
      item_type: 'news_release',
  ...
  const calDoc = await ctx.fetchDocument(calUrl, { docType: 'feed', title: 'Calendar — All' });
  ...
  for (const it of calItems) {
    const occurredAt = chinoDateTimeToIso(it.extra.EventDates ?? '', (it.extra.EventTimes ?? '').split('-')[0]);
    ctx.insertItem({
