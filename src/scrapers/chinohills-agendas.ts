...
function extractSequentialItems(rawText: string): {
  ...
  while ((im = itemRe.exec(text))) {
    rawMatches.push({ num: parseInt(im[1], 10), start: im.index, end: itemRe.lastIndex });
  }
  ...
  const items: ExtractedItem[] = sequential.map((m, i) => {
    const stop = i + 1 < sequential.length ? sequential[i + 1].start : text.length;
    const chunk = text.slice(m.end, stop).replace(pageMarkerRe, '\n');
    const firstLine = chunk.split('\n').map((l) => l.trim()).find((l) => l.length > 0) ?? '';
    const body = chunk.replace(/\s+/g, ' ').trim();
    return { num: m.num, page: pageForIndex(m.start), title: firstLine.slice(0, 120), body };
  });

  return {
    items,
    rawMatchCount: rawMatches.length,
    stoppedAtNum: sequential.length > 0 ? sequential[sequential.length - 1].num : null,
  };
}
...
function countHtmlTopLevelItems($: cheerio.CheerioAPI): number {
  ...
  const nums = lines.filter((l) => /^\d{1,3}\.$/.test(l)).map((l) => parseInt(l, 10));
...
async function run(ctx: ScraperContext, args: string[] = []): Promise<void> {
  ...
  const hubDoc = await ctx.fetchDocument(AGENDAS_MINUTES_URL, { docType: 'listing', title: 'Agendas & Minutes' });
  ...
  if (councilLinks?.mostRecentAgenda) {
    try {
      await ctx.fetchDocument(councilLinks.mostRecentAgenda, { docType: 'agenda', title: 'City Council — Most Recent Agenda (WebLink)' });
      ctx.note(`Step 3 — UNEXPECTED: ${councilLinks.mostRecentAgenda} (Laserfiche WebLink) was fetchable; robots.txt may have changed since 2026-08-11 — re-verify.`);
    } catch (err) {
      ctx.note(
  ...
  const monthsToCheck = [
    ...
    { month: now.getUTCMonth() === 0 ? 12 : now.getUTCMonth(), year: now.getUTCMonth() === 0 ? now.getUTCFullYear() - 1 : now.getUTCFullYear() },
  ];
  ...
  for (const { month, year } of monthsToCheck) {
    ...
    const listingDoc = await ctx.fetchDocument(listingUrl, { docType: 'listing', title: `Agenda Quick — ${year}-${month} meetings` });
    const $listing = cheerio.load(listingDoc.body.toString('utf8'));
    const meetings = parseAqListing($listing);
    allMeetings.push(...meetings);
    ctx.note(`Step 4 — ${listingUrl}: ${meetings.length} meeting(s) with a posted agenda found (${meetings.map((m) => `${m.dateIso} ${m.bodyName}`).join('; ') || 'none'}).`);
  }

  const byRecency = [...allMeetings].sort((a, b) => b.dateMs - a.dateMs);
  const mostRecentCouncil = byRecency.find((m) => /city council/i.test(m.bodyName));
  const mostRecentCommission = byRecency.find((m) => /commission/i.test(m.bodyName));
  const selected = [mostRecentCouncil, mostRecentCommission].filter((m): m is AqMeeting => !!m);
  if (!mostRecentCommission) {
    ctx.note('No commission-type meeting (name matching /commission/i) found in the current or prior month\'s AgendaQuick listing — proceeding with City Council only.');
  }
