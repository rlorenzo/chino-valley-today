...
function extractNewsflashDetail($: cheerio.CheerioAPI): { text: string; selector: string } {
  ...
  return { text: fallback, selector: '#moduleContent (fallback — generic layout, lower confidence)' };
}
...
async function run(ctx: ScraperContext): Promise<void> {
  ...
  const allDoc = await ctx.fetchDocument(allFeedUrl, { docType: 'feed', title: 'News Flash — All' });
  ...
  const collected: Array<{ item: FeedItem; category: string; feedUrl: string; documentId: number }> = [];
  const seenGuids = new Set<string>();
  for (const cat of NEWSFLASH_CATEGORIES) {
    const url = `${BASE}/RSSFeed.aspx?ModID=1&CID=${cat.cid}`;
    const doc = await ctx.fetchDocument(url, { docType: 'feed', title: `News Flash — ${cat.name}` });
    const items = parseRssItems(doc.body.toString('utf8'));
    if (items.length === 0) {
      ctx.note(`News Flash category "${cat.name}" (CID=${cat.cid}) currently has 0 items.`);
      continue;
    }
    for (const it of items) {
      collected.push({ item: it, category: cat.name, feedUrl: url, documentId: doc.documentId });
  ...
  if (uncategorized.length > 0) {
    ...
    for (const it of uncategorized) {
      collected.push({ item: it, category: 'Uncategorized (via All feed)', feedUrl: allFeedUrl, documentId: allDoc.documentId });
    }
  } else {
    ctx.note('Every item in the "All" feed is accounted for by a named category — no uncategorized items this run.');
  }
  ...
  if (collected.length > 0) {
    ...
    const detailDoc = await ctx.fetchDocument(sample.item.link, { docType: 'news_release', title: sample.item.title });
  ...
  for (const c of collected) {
    ctx.insertItem({
      document_id: resolveDocumentId(ctx, c.documentId, c.item.guid, 'news_release'),
