...
function parseListing(html: string): VideoRow[] {
  ...
  $('tr').each((_i, tr) => {
    ...
    let title = '';
    cells.eq(0).find('a[href^="/videos/"]').each((_j, a) => {
...
function sortByDateDesc(rows: VideoRow[]): VideoRow[] {
  return rows
    ...
    .filter((x): x is { r: VideoRow; d: { iso: string; time: number } } => x.d !== null)
...
async function verifyTsDeepLink(ctx: ScraperContext, videoUrl: string): Promise<boolean> {
  try {
    ...
    const hasHardcodedSeek =
      body.includes(`.currentTime(${TS_PROBE_SECONDS})`) || body.includes(`.seek(${TS_PROBE_SECONDS})`);
...
const scraper: ScraperDef = {
  ...
  async run(ctx) {
    ...
    if (rows.length === 0) {
      ctx.note('No parsable/dated video rows found in the listing — nothing to ingest.');
      return;
    }

    // --- 2. Walk newest-first until one has a real transcript. ---
    let target: VideoRow | null = null;
    let videoDoc: Awaited<ReturnType<ScraperContext['fetchDocument']>> | null = null;
    ...
    if (!target || !videoDoc || !$video) {
      ctx.note(
        `Tried ${tried} most-recent meeting(s) in the listing and none had a machine transcript — nothing to ingest. ` +
          `Meetings checked: ${rows.slice(0, tried).map((r) => `${r.id} (${r.dateText})`).join(', ')}.`
      );
      return;
    }
    ...
    const meetingDate = parseDateText(pageTitle) ?? parseDateText(target.dateText);
    ...
    const probes: Array<[string, string]> = [
      ...
      [`${HOST}/captions/${vid}`, 'guessed /captions/<id> (PLAN.md pattern guess)'],
    ];
    ...
    for (const [url, label] of probes) {
      try {
        ...
        probeResults.push(`${url} (${label}) -> HTTP ${res.status}, content-type=${res.contentType ?? 'n/a'}`);
        if (url.endsWith('/transcript')) {
          transcriptTxtUrl = url;
          transcriptTxtOk = res.ok;
        }
      } catch (err) {
        probeResults.push(`${url} (${label}) -> ERROR: ${(err as Error).message}`);
      }
    }
    ctx.note(
    ...
    if (segments.length > TRUNCATE_ABOVE) {
      ...
      ctx.note(`Volume bound not triggered: ${segments.length} segments is under the ~1500 threshold — storing all of them.`);
    }

    for (let i = 0; i < toStore.length; i++) {
      const seg = toStore[i];
      const startRounded = Math.round(seg.start);
      const sourceUrl = tsSupported ? `${target.url}?ts=${startRounded}` : target.url;
      ctx.insertItem({
