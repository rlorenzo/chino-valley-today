...
function parseMeetingRows($: cheerio.CheerioAPI, pageUrl: string, documentId: number): MeetingRow[] {
  ...
  $('table').each((_, table) => {
    ...
    $table.find('tbody tr').each((_, tr) => {
      ...
      const agendaUrl = agendaHref ? new URL(agendaHref, pageUrl).toString() : null;
      const minutesUrl = minutesHref ? new URL(minutesHref, pageUrl).toString() : null;
      const videoUrl = videoHref ? new URL(videoHref, pageUrl).toString() : null;
      rows.push({
        date,
        type,
        agendaUrl,
...
function discoverPriorYearUrls($: cheerio.CheerioAPI, pageUrl: string): string[] {
  const urls: string[] = [];
  $('a').each((_, a) => {
    const $a = $(a);
    if (!/Meeting Dates,?\s*Agendas,\s*Minutes and Videos/i.test($a.text())) return;
...
function splitAgendaItems(fullText: string): { items: AgendaItem[]; boundedFallback: boolean } {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    let bodyRaw = region.slice(cur.contentStart, next ? next.idx : region.length).trim();
...
async function ingestAgendaPdf(ctx: ScraperContext, row: MeetingRow): Promise<void> {
  ...
  for (const item of items) {
    ctx.insertItem({
      ...
      source_url: item.page ? `${row.agendaUrl}#page=${item.page}` : row.agendaUrl,
      item_type: 'agenda_item',
      external_id: `${row.date}-${item.n}`,
      title: item.title,
      body: item.body,
      occurred_at: row.date,
      meta: { itemNumber: item.n, packetPage: item.page, meetingType: row.type },
    });
  }
  ctx.note(
...
const scraper: ScraperDef = {
  ...
  async run(ctx) {
    ...
    const priorYearUrls = discoverPriorYearUrls($entry, MEETINGS_URL).slice(0, MAX_PRIOR_YEAR_PAGES);
    for (const yearUrl of priorYearUrls) {
      ...
      ctx.note(`Prior-year listing page ${yearUrl}: ${yrRows.length} meeting row(s).`);
    ...
    const agendaHosts = new Set(allRows.filter((r) => r.agendaUrl).map((r) => new URL(r.agendaUrl!).host));
    ctx.note(
    ...
    const minutesHosts = new Set(allRows.filter((r) => r.minutesHost).map((r) => r.minutesHost));
    ctx.note(
    ...
    for (const row of allRows) {
      const sourceUrl = row.agendaUrl ?? row.videoUrl ?? row.minutesUrl ?? row.listingUrl;
      ctx.insertItem({
    ...
    const candidates = allRows.filter((r) => r.agendaUrl && classifyHost(r.agendaUrl) === 'smartsites');
