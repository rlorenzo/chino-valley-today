...
function storeAlerts(ctx: Parameters<ScraperDef['run']>[0], documentId: number, features: AlertFeature[]) {
...
const scraper: ScraperDef = {
  ...
  async run(ctx) {
    ...
    const active = await ctx.fetchDocument(`https://api.weather.gov/alerts/active?zone=${ZONE}`, {
      docType: 'alert',
      title: `Active NWS alerts for ${ZONE}`,
      skipRobots: true,
    });
    const activeFeed = JSON.parse(active.body.toString('utf8')) as { features?: AlertFeature[] };
    ...
    const recent = await ctx.fetchDocument(`https://api.weather.gov/alerts?zone=${ZONE}&limit=10`, {
      docType: 'alert',
      title: `Recent NWS alerts for ${ZONE}`,
      skipRobots: true,
    });
    const recentFeed = JSON.parse(recent.body.toString('utf8')) as { features?: AlertFeature[] };
    const recentFeatures = recentFeed.features ?? [];
    ctx.note(`${recentFeatures.length} recent alert(s) (incl. expired) stored for sample purposes`);
    storeAlerts(ctx, recent.documentId, recentFeatures);

    ctx.note('api.weather.gov requires a User-Agent header; supports ETag (If-None-Match honored).');
  },
};
