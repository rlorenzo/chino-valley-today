...
function pacificToIso(
	...
	const utcMs =
		Date.UTC(year, month1to12 - 1, day, hour24, minute) +
		offsetHours * 3600 * 1000;
	return new Date(utcMs).toISOString();
}

const CORONER_DATE_RE =
	/On\s+\w+day,\s*(\d{2})\/(\d{2})\/(\d{4}),\s*at\s*(\d{1,2}):(\d{2})\s*(a\.m\.|p\.m\.)/i;
...
async function run(ctx: ScraperContext): Promise<void> {
	...
	const postsProbe = await ctx.fetchRaw(
		`${BASE}/wp-json/wp/v2/posts?per_page=5`,
	);
	let postsCount: number | null = null;
	if (postsProbe.ok) {
		try {
			postsCount = (JSON.parse(postsProbe.body.toString("utf8")) as unknown[])
				.length;
		} catch {
	...
	const categoriesProbe = await ctx.fetchRaw(
		`${BASE}/wp-json/wp/v2/categories?per_page=100&search=chino`,
	);
	let chinoHillsCategoryFound = false;
	let chinoHillsCategoryCount: number | null = null;
	if (categoriesProbe.ok) {
		try {
			const cats = JSON.parse(categoriesProbe.body.toString("utf8")) as Array<{
				slug: string;
				name: string;
				count: number;
			}>;
	...
	ctx.note(
		`Station taxonomy verdict: WordPress category "Chino Hills" (slug chino-hills) ${
			chinoHillsCategoryFound
				? `exists, post count = ${chinoHillsCategoryCount}`
				: "was not found"
	...
	const feedDoc = await ctx.fetchDocument(FEED_URL, {
		docType: "feed",
		title: "SB Sheriff site-wide RSS feed",
	});
	...
	let feedItemCount = 0;
	try {
		const parsed = xmlParser.parse(feedDoc.body.toString("utf8")) as Record<
			string,
			unknown
		>;
		...
		feedItemCount =
			rawItems == null ? 0 : Array.isArray(rawItems) ? rawItems.length : 1;
	} catch (err) {
		ctx.note(
			`Feed at ${FEED_URL} failed to parse as RSS: ${(err as Error).message}`,
		);
	}
	ctx.note(
	...
	const stationDoc = await ctx.fetchDocument(CHINO_HILLS_STATION_URL, {
		docType: "listing",
		title: "Chino Hills patrol station page",
	});
	const $station = cheerio.load(stationDoc.body.toString("utf8"));
	const stationNixleHref =
		$station('a[href*="nixle"]').first().attr("href") ?? null;
	ctx.note(
	...
	try {
		...
		if (nixleRaw.ok) {
			...
			ctx.note(
				`Nixle disposition: channel page returned HTTP ${nixleRaw.status}; skipping the cadence count for this run.`,
			);
		}
	} catch (err) {
		ctx.note(
			`Nixle disposition: best-effort cadence check failed (${(err as Error).message}); not fetched via fetchDocument, nothing archived, no impact on ingestion.`,
		);
	}

	// --- Coroner press releases: the one live, non-Nixle, current source ---
	const coronerDoc = await ctx.fetchDocument(CORONER_URL, {
		docType: "news_release",
		title: "San Bernardino County Coroner press releases",
	});
	...
	const chinoHillsMatches = coronerItems.filter((it) =>
		CHINO_HILLS_RE.test(it.body),
	);
	ctx.note(
	...
	if (missingDatePattern > 0) {
		ctx.note(
			`${missingDatePattern} of ${toIngest.length} ingested coroner item(s) did not match the expected "On <Weekday>, MM/DD/YYYY, at H:MM a.m./p.m." date pattern; occurred_at left null for those.`,
		);
	}
	...
	ctx.note(
		"Full-text vs teaser: N/A in the RSS sense (the feed is empty) — the Coroner page embeds full release text directly in the page body, no separate detail page or teaser/full split exists for this source.",
	);
}
