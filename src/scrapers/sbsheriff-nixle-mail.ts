...
const NIXLE_PERMALINK_RE = /https?:\/\/(?:www\.)?nixle\.us\/([A-Za-z0-9]{3,12})\b/;

export function extractNixlePermalink(text: string): { url: string; code: string } | null {
...
export function messageToItemDraft(msg: NixleMessageFields): NixleItemDraft | null {
  const permalink = extractNixlePermalink(msg.text ?? '') ?? extractNixlePermalink(msg.html ?? '');
  ...
  const priorityMatch = subject.match(/^(Alert|Advisory|Community|Traffic)\s*:/i);
...
async function run(ctx: ScraperContext): Promise<void> {
  ...
  const alias = (process.env.NIXLE_MAIL_ALIAS ?? 'chinovalleytoday+nixle@gmail.com').toLowerCase();
  ...
  try {
    ...
    try {
      ...
      for (const uid of uids) {
        ...
        const isForUs = headerAddresses(mail).includes(alias) || fromText.includes('nixle');
        if (!isForUs) continue;
        matched++;
