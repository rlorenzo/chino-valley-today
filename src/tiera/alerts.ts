...
import { cleanTitle, localMeetingDate, mdEscape, mdLink, slugify } from './util.ts';
...
export function generateAlerts(db: Db, now: Date): GenResult {
  ...
  for (const row of items) {
    ...
    const localDate = row.occurred_at ? localMeetingDate(row.occurred_at) : null;
    ...
    const lines: string[] = [];
    if (typeof meta.severity === 'string') lines.push(`- **Severity:** ${mdEscape(meta.severity)}`);
    if (typeof meta.urgency === 'string') lines.push(`- **Urgency:** ${mdEscape(meta.urgency)}`);
    if (typeof meta.areaDesc === 'string') lines.push(`- **Area:** ${mdEscape(meta.areaDesc)}`);
    if (typeof meta.effective === 'string') lines.push(`- **Effective:** ${mdEscape(meta.effective)}`);
    lines.push(`- **Ends:** ${mdEscape(ends)}`);
    lines.push('');
    if (row.body && row.body.trim()) {
      lines.push(`> ${mdEscape(row.body.replace(/\s+/g, ' ').trim())}`, '');
    }
    lines.push(mdLink('Official alert data (National Weather Service)', row.source_url));
