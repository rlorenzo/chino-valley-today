...
import { cleanTitle, dedupeByKey, humanDateFromLocal, isFutureOccurredAt, localMeetingDate, mdEscape, mdLink, slugify } from './util.ts';
...
function agendaBodyName(row: ItemRow, meta: Record<string, unknown>): string | null {
  if (row.source_key === 'chino-legistar' && typeof meta.eventBodyName === 'string') return meta.eventBodyName;
  if (
    (row.source_key === 'chino-agendacenter' || row.source_key === 'chinohills-agendas') &&
...
function findMatchingAgendaItems(agendaItems: ItemRow[], localDate: string, bodyName: string): ItemRow[] {
...
function agendaListMarkdown(items: ItemRow[]): string {
  const sorted = [...items].sort((a, b) => agendaSortKey(a) - agendaSortKey(b) || a.id - b.id);
  const lines = sorted
    .map((row) => {
...
function chinoCalendarBodyName(rawTitle: string): { cancelled: boolean; body: string } {
  let t = rawTitle.trim();
  let cancelled = false;
...
function genChinoCalendarPreviews(db: Db, now: Date, crossRefAgendaItems: ItemRow[]): GenResult {
  const events = queryItems(db, { sourceKeys: ['chino-news-rss'], itemTypes: ['event'] });
  const notes: string[] = [];
  const posts: NewPost[] = [];
  let futureCount = 0;
  ...
  for (const row of events) {
    ...
    if (!localDate) {
      notes.push(`Skipped calendar event "${title}": occurred_at "${row.occurred_at}" did not parse to a calendar date.`);
      continue;
    }

    const { cancelled, body } = chinoCalendarBodyName(title);
    const meta = parseMeta(row.meta);
    const eventDates = typeof meta.eventDates === 'string' ? meta.eventDates : null;
    const eventTimes = typeof meta.eventTimes === 'string' ? meta.eventTimes : null;
    const location = typeof meta.location === 'string' ? meta.location : null;

    const matched = cancelled ? [] : findMatchingAgendaItems(crossRefAgendaItems, localDate, body);
    ...
    const lines: string[] = [];
    if (cancelled) lines.push(`**This meeting has been CANCELLED** (${cite('city calendar')}).`, '');
    const detailLines: string[] = [];
    if (eventDates) detailLines.push(`- **Date:** ${mdEscape(eventDates)} (${cite('calendar')})`);
    if (eventTimes) detailLines.push(`- **Time:** ${mdEscape(eventTimes)} (${cite('calendar')})`);
    if (location) detailLines.push(`- **Location:** ${mdEscape(location)} (${cite('calendar')})`);
    ...
    if (!cancelled) {
      ...
      if (agendaMd) {
        ...
        lines.push(`_No agenda had been posted to our records as of publish time (${cite('calendar')})._`);
      }
    }
    ...
    posts.push({
      ...
      tier: 'A',
      title: cancelled ? `CANCELLED: ${body} — ${humanDate}` : `Meeting Preview: ${body} — ${humanDate}`,
      bodyMd: lines.join('\n'),
...
function genCvusdPreviews(db: Db, now: Date): GenResult {
  const events = queryItems(db, { sourceKeys: ['cvusd-board'], itemTypes: ['event'] });
  const notes: string[] = [];
  const posts: NewPost[] = [];
  let futureCount = 0;
  ...
  for (const row of events) {
    ...
    const meta = parseMeta(row.meta);
    const meetingType = typeof meta.meetingType === 'string' ? meta.meetingType : null;
    const agendaUrl = typeof meta.agendaUrl === 'string' ? meta.agendaUrl : null;
    ...
    if (meetingType) {
      lines.push(`- **Meeting type:** ${mdEscape(meetingType)} (${mdLink('board meeting page', row.source_url)})`);
    }
    lines.push('', mdLink('Board meeting page', row.source_url));
    if (agendaUrl && agendaUrl !== row.source_url) lines.push('', mdLink('Agenda', agendaUrl));
    ...
    lines.push(
      ...
      "_No agenda item text is in our records for this meeting — CVUSD's agenda PDF host " +
        `currently blocks automated fetching (robots.txt); see the ${
          agendaUrl ? mdLink('agenda', agendaUrl) : mdLink('board meeting page', row.source_url)
        }._`
    );

    const sourceUrls = dedupeByKey(
      [row.source_url, agendaUrl].filter((u): u is string => !!u).map((u) => ({ url: u })),
      (r) => r.url
    ).map((r) => r.url);
...
function genLegistarPreviews(now: Date, legistarAgendaItems: ItemRow[]): GenResult {
  const notes: string[] = [];
  const posts: NewPost[] = [];
  ...
  for (const [eventId, rows] of groups) {
    ...
    if (!localDate) {
      notes.push(`Skipped Legistar EventId ${eventId}: occurred_at "${occurredAt}" did not parse.`);
      continue;
    }
    const meta0 = parseMeta(rows[0]!.meta);
    const eventBodyName = typeof meta0.eventBodyName === 'string' ? meta0.eventBodyName : `Meeting ${eventId}`;
...
function genChinohillsAgendaPreviews(now: Date, chinohillsAgendaItems: ItemRow[]): GenResult {
  const notes: string[] = [];
  const posts: NewPost[] = [];
  ...
  for (const [seq, rows] of groups) {
    ...
    const meta0 = parseMeta(rows[0]!.meta);
    const bodyName = typeof meta0.body === 'string' ? meta0.body : `Meeting seq ${seq}`;
...
export function generateMeetingPreviews(db: Db, now: Date): GenResult {
  const legistarAgendaItems = queryItems(db, { sourceKeys: ['chino-legistar'], itemTypes: ['agenda_item'] });
  const agendacenterAgendaItems = queryItems(db, { sourceKeys: ['chino-agendacenter'], itemTypes: ['agenda_item'] });
  const chinohillsAgendaItems = queryItems(db, { sourceKeys: ['chinohills-agendas'], itemTypes: ['agenda_item'] });
  const crossRefAgendaItems = [...legistarAgendaItems, ...agendacenterAgendaItems, ...chinohillsAgendaItems];
