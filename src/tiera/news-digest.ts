...
import { dedupeByKey, isoWeekForNow, mdEscape, mdLink, truncateTeaser, withinLastDays } from './util.ts';
...
export function generateNewsDigest(db: Db, now: Date): GenResult {
  const items = queryItems(db, { sourceKeys: ['chino-news-rss', 'chinohills-news-rss'], itemTypes: ['news_release'] });
  const notes: string[] = [];
  const posts: NewPost[] = [];

  const inWindow = items.filter((r) => withinLastDays(r.occurred_at, now, WINDOW_DAYS) && r.title?.trim());
  const deduped = dedupeByKey(inWindow, (r) => r.source_url);
  const sorted = [...deduped].sort((a, b) => (b.occurred_at ?? '').localeCompare(a.occurred_at ?? ''));
  ...
  notes.push(
    `${sorted.length} news_release item(s) in the last ${WINDOW_DAYS} days` +
      (dupesRemoved > 0 ? ` (${dupesRemoved} duplicate row(s) deduped by source_url)` : '') +
