...
export function isFutureOccurredAt(occurredAt: string | null, now: Date): boolean {
...
export function withinLastDays(occurredAt: string | null, now: Date, days: number): boolean {
  if (!occurredAt) return false;
  const t = new Date(occurredAt).getTime();
